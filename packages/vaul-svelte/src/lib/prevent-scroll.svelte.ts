...
import { type ReadableBoxedValues, addEventListener } from "svelte-toolbelt";

type PreventScrollOptions = ReadableBoxedValues<{
...
const visualViewport =
	typeof document !== "undefined" && typeof window !== "undefined" && window.visualViewport;
 ...
 */
export function usePreventScroll({ isDisabled }: PreventScrollOptions) {
	$effect(() => {
...
function setStyle(element: HTMLElement, style: string, value: string) {
	const cur = element.style[style as keyof CSSStyleDeclaration] as string;
	// eslint-disable-next-line ts/no-explicit-any
